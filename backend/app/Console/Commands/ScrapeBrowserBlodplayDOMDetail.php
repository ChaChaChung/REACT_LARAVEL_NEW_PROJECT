...
class ScrapeBrowserBlodplayDOMDetail extends Command
{
    ...
    /**
     * 獲取 Cookies（從瀏覽器）
     * @param string|null $accountNumber 帳號（未使用，保留以保持接口一致性）
     * @return array 返回 cookies 陣列
     */
    private function getCookies($accountNumber = null)
    {
        $this->info('1. Getting cookies from browser...');
        
        if (!$this->checkNodeJs()) {
            return [];
        }

        $scriptPath = $this->createCookiesFetchScript();
        $result = $this->runCookiesFetchScript($scriptPath);

        if ($result && !empty($result['cookies'])) {
            $this->info('✅ Cookies obtained: ' . count($result['cookies']) . ' cookies');
            return $result['cookies'];
        }

        $this->warn('⚠️  No cookies obtained from browser');
        return [];
    }
    ...
    /**
     * 從 API 獲取所有頁面的數據
     * @param array $cookies Cookies 陣列（用於認證）
     * @param int $timestamp 時間戳（毫秒）
     * @param string $accountNumber 帳號
     * @return array|null
     */
    private function fetchAllDataFromApi($cookies, $timestamp, $accountNumber)
    {
        ...
        do {
            ...
            if (!$timestamp || $timestamp <= 0) {
                $this->error("❌ Invalid timestamp: {$timestamp}");
                return null;
            }
            ...
            if (!empty($cookies)) {
                ...
                if ($currentPage === 1) {
                    $this->line("   🔍 Debug - Using " . count($cookies) . " cookies for authentication");
                }
            } else {
                $this->error("❌ No cookies available for authentication");
                return null;
            }
            ...
            if ($currentPage === 1) {
                ...
                $this->line("   🔍 Debug - Timestamp: " . $timestamp . " (" . date('Y-m-d H:i:s', $timestamp / 1000) . ")");
                if (!empty($cookies)) {
                    $this->line("   🔍 Debug - Cookies count: " . count($cookies));
                }
            }
            
            // 使用 GET 請求，查詢參數作為第二個參數
            try {
            ...
            if (!$response->successful()) {
                ...
                if ($response->status() === 400) {
                    $this->line("   🔍 Debug - Request URL: " . $baseUrl);
                    $this->line("   🔍 Debug - Request params: " . json_encode($queryParams, JSON_UNESCAPED_UNICODE));
                    $this->line("   🔍 Debug - Full URL with params: " . $baseUrl . '?' . http_build_query($queryParams));
                }
     ...
     */
    private function saveApiData($data, $date)
    {
     ...
     * @return string
     */
    private function createCookiesFetchScript()
    {
        $domain = env('BLODPLAY_AGENT_DOMAIN', '');
        $loginCode = $this->generateBlodplayPuppeteerLoginCode('page');
        ...
        $domainJs = json_encode($domainUrl);
        ...
        $script = <<<JS
            ...
            async function getCookies() {
                ...
                try {
                    ...
                    // 導航到目標域名
                    const domainUrl = $domainJs;
                    if (domainUrl && domainUrl !== '' && domainUrl !== 'null') {
                        console.log('🌐 Navigating to domain: ' + domainUrl);
                        try {
                    ...
                    await new Promise(resolve => setTimeout(resolve, 3000));
                        ...
                        console.log('⚠️  No domain URL provided, skipping navigation');
                    }
                    ...
                    // 調試：顯示所有 cookies
                    console.log('🍪 All cookies:', pageCookies.map(c => c.name).join(', '));
                    console.log('🍪 Total cookies: ' + pageCookies.length);
                    
                    // 保存結果
                    fs.writeFileSync('cookies_result.json', JSON.stringify({
                            success: true,
                        cookies: pageCookies,
                        count: pageCookies.length
                        }, null, 2));
        ...
        $scriptPath = storage_path('app/temp/fetch_cookies.js');
     ...
     * @return array|null
     */
    private function runCookiesFetchScript($scriptPath)
    {
        ...
        // 顯示腳本執行的輸出（用於調試）
        if ($result->output()) {
            $this->line("Script output: " . $result->output());
        }

        $cookiesFile = $workingDir . '/cookies_result.json';
        
        // 即使腳本執行失敗，也嘗試讀取結果文件（可能包含錯誤信息）
        if (file_exists($cookiesFile)) {
            ...
            if ($data && isset($data['success']) && $data['success']) {
                ...
                $this->error("❌ Cookies fetch failed: " . $data['error']);
                return null;
            }
        }

        if ($result->failed()) {
            $this->error("❌ Cookies fetch failed");
            $this->line("Error: " . $result->errorOutput());
            if (file_exists($cookiesFile)) {
                $content = file_get_contents($cookiesFile);
                $errorData = json_decode($content, true);
                if ($errorData && isset($errorData['error'])) {
                    $this->line("Error from script: " . $errorData['error']);
                }
            }
            return null;
        }

        $this->error("❌ Cookies result file not found");
        return null;
    }
        ...
        $script = <<<JS
            ...
            async function scrapeDOMContent() {
                ...
                try {
                    ...
                    await takeScreenshot('00_after_navigation_stop');
                    ...
                    fs.writeFileSync('scraped_result.json', JSON.stringify({
                        ...
                        url: '$url',
                        message: 'Stopped after navigation - screenshot taken',
                        screenshot: 'step_00_after_navigation_stop_*.png'
                    }, null, 2));
                    
                    console.log('✅ Navigation completed. Stopping execution as requested.');
                    return;
