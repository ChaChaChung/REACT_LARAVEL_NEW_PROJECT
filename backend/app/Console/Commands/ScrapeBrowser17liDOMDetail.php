 ...
 * 瀏覽器 DOM 內容爬蟲命令
 */
class ScrapeBrowser17liDOMDetail extends Command
{
    use HasAgentAuth;
     ...
     * 命令簽名和參數定義
     * @var string
     * 執行方式：php artisan agent:scrape-17li-dom-detail {url}
     * {url} - 要爬取的目標網址（必需參數）
     ...
     */
    protected $signature = 'agent:scrape-17li-dom-detail {url} {date_start?} {date_end?} {platform?} {account_number?} {--concurrency=4}';

    /**
     * 命令描述
     * @var string
     */
    protected $description = 'Scrape content from 17LI DOM elements using browser automation with detailed information';
    ...
    private function createPuppeteerScript($url, $date_start = null, $date_end = null, $account_number = null, $platform = null, $concurrency = 4)
    {
        ...
        $cookiesCodeForPage = $this->generate17liPuppeteerCookiesCode('page');
        // 獲取認證 cookies 程式碼片段（併發頁面用）
        $cookiesCodeForNewPage = $this->generate17liPuppeteerCookiesCode('newPage');
