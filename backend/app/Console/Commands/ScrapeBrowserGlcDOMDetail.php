 ...
 * 瀏覽器 DOM 內容爬蟲命令
 */
class ScrapeBrowserGlcDOMDetail extends Command
{
    use HasAgentAuth;
     ...
     * @var string
     */
    protected $description = 'Scrape content from GLC DOM elements using browser automation with detailed information';
    ...
    private function createPuppeteerScript($url, $date_start = null, $date_end = null, $concurrency = 4)
    {
        ...
        $cookiesCodeForPage = $this->generateGlcPuppeteerCookiesCode('page');
                ...
                try {
                    ...
                    const extractTableData = async (pageObject) => {
                        return await pageObject.evaluate(() => {
                            ...
                            // Element UI 表格可能有多個部分：el-table__header 和 el-table__body
                            ...
                            // 如果還是找不到，嘗試查找任何包含 el-table 的元素
                            if (!table && !headerTable && !bodyTable) {
                                const elTableElement = document.querySelector('.el-table, .el-table__header, .el-table__body, [class*="el-table"]');
                                if (elTableElement) {
                                    // 向上查找最近的 table 元素
                                    ...
                                    if (parent && parent.tagName === 'TABLE') {
                                        if (parent.className.includes('el-table__header')) {
                                            headerTable = parent;
                                        } else if (parent.className.includes('el-table__body')) {
                                            bodyTable = parent;
                                        } else {
                                            table = parent;
                                        }
                                    } else if (elTableElement.tagName === 'TABLE') {
